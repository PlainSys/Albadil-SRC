...
var lorbb := preload("res://gorbnew.tscn")

onready var spawnTimer := $SpawnTimer
var nextSpawnTimer := 3.0
...
func _on_SpawnTimer_timeout():
	...
	if randf() < 0.3:
		var lorb := lorbb.instance()
		lorb.position = Vector2(xPos, position.y)
		get_tree().current_scene.add_child(lorb)
	elif randf() < 0.2:
		var gorb := pertest.instance()
		gorb.position = Vector2(xPos, position.y)
		get_tree().current_scene.add_child(gorb)
	else:
